...
class ColorManager:
    """مدير الألوان المتغيرة لكل رسالة"""
    def __init__(self):
        self.current_index = 0
    
    def get_next_color(self):
        """الحصول على اللون التالي في الدورة"""
        color = COLOR_EMOJIS[self.current_index]
        self.current_index = (self.current_index + 1) % len(COLOR_EMOJIS)
        return color
...
def send_welcome(message):
    ...
    fancy_text = (
        "<b>❍<u>𝐖𝐞𝐥𝐜𝐨𝐦 𝐭𝐨 𝑀𝐸𝐷𝑂 𝒃𝒐𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>𝐅𝐚𝐬𝐭  • 𝐒𝐞𝐜𝐮𝐫𝐞  • 𝐨𝐧𝐥𝐢𝐧𝐞</u></b>\n\n"
        "<b>🎓 <u>𝐎𝐰𝐧𝐞𝐫</u>  • <a href='tg://user?id=8300713565'>𝐻𝐴𝑀𝑂</a></b>\n\n"
        "<b>────────────────────</b>\n"
...
def handle_country_selection(call):
    ...
    # زر الجروب في الأعلى
    markup.add(types.InlineKeyboardButton("𝑉𝑖𝑒𝑤 𝑂𝑡𝑝👀", url="https://t.me/OTPHIGUotp"))
...
def back_to_countries(call):
    ...
    fancy_text = (
        "<b>❍<u>𝐖𝐞𝐥𝐜𝐨𝐦 𝐭𝐨 𝐻𝐴𝑀𝑂 𝒃𝒐𝒕 </u>❍</b>\n\n"
        "<b>🔋 <u>𝐅𝐚𝐬𝐭  • 𝐒𝐞𝐜𝐮𝐫𝐞  • 𝐨𝐧𝐥𝐢𝐧𝐞</u></b>\n\n"
        "<b>🎓 <u>𝐎𝐰𝐧𝐞𝐫</u>  • <a href='tg://user?id=8300713565'>𝑀𝐸𝐷𝑂</a></b>\n\n"
        "<b>────────────────────</b>\n"
        "<b><u>اخـتـر الــدولـة الـتـي تـريـدهـا مـن الـزر الاسـفـل</u> ⬇️</b>"
    )
...
# ======================
# 🎨 تنسيق الرسالة الجديد مع الألوان المتغيرة
# ======================
def format_message(date_str, number, sms):
    ...
    # الحصول على اللون المتغير
    color = color_manager.get_next_color()

    # التنسيق الجديد مع الألوان المتغيرة
    message = (
        f"{color} 𝑂𝑇𝑃 𝑅𝐸𝐶𝐸𝐼𝑉𝐸𝐷 {color}\n\n"
        f"╭───────────────╮\n"
        f"│ {country_flag} {service} {masked_num}\n"
        f"╰───────────────╯\n\n"
        f"🔑 Code: {otp_code}"
    )
    return message, otp_code, color
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("𝑂𝑤𝑛𝑒𝑟🌪", url="https://t.me/lillala24"),
                types.InlineKeyboardButton("𝐶ℎ𝑎𝑛𝑛𝑒𝑙🫀", url="https://t.me/OTPHIGUotp")
            )
            bot.send_message(
                user_id,
    ...
    # إرسال نفس الرسالة للجروب مع الألوان المتغيرة
    text, otp, color = format_message(date_str, number, sms)
    send_to_telegram_group(text, otp_code, color)
...
# ======================
# 🎨 دالة الإرسال للجروب مع الألوان المتغيرة
# ======================
def send_to_telegram_group(text, otp_code, color="🔴"):
    success_count = 0
    try:
        # بناء لوحة الأزرار مع الألوان المتغيرة
        keyboard = {
            "inline_keyboard": [
                # زر نسخ الكود باللون المتغير
                [{"text": f"{color} ┃ نسخ الكود: {otp_code}", "copy_text": {"text": str(otp_code)}}],
                
                # الصف الثاني: زر القناة وزر البوت بنفس اللون
                [
                    {"text": f"{color} القناة", "url": "https://t.me/FKFGkF_OTP"},
                    {"text": f"{color} البوت", "url": "https://t.me/NEW210BOT"}
                ],
                
                # الصف الثالث: زر المطور
                [
                    {"text": "𝒔𝒖𝒑𝒑𝒐𝒓𝒕🧑‍💻️", "url": "https://t.me/lillala24"}
                ]
            ]
        }
    except Exception as e:
    ...
    for chat_id in CHAT_IDS:
        try:
            ...
            if resp.status_code == 200:
                print(f"[+] تم إرسال الرسالة بنجاح إلى: {chat_id} | اللون: {color}")
                success_count += 1
